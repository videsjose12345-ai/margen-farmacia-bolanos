...
import { fmt$, fmtDate, fmtN, fmtPct, shortUbic, longUbic, precioSugerido, mgClass } from '../lib/format'
...
function mgBadge(m: number) { return mgClass(m) === 'danger' ? 'mg-badge mg-danger' : mgClass(m) === 'warning' ? 'mg-badge mg-warning' : 'mg-badge mg-success' }
function mgTextCls(m: number) { return mgClass(m) === 'danger' ? 'text-red-500 font-bold' : mgClass(m) === 'warning' ? 'text-yellow-600 font-bold' : 'text-green-600 font-bold' }
...
export default function AnalysisPage({ appState, triggerFile }: Props) {
  ...
  if (!state.actual) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
        ...
        <div className="text-sm" style={{ color: 'var(--text2)' }}>Cargue un archivo Excel para ver el análisis. Solo se muestran productos con saldo &gt; 0.</div>
        <button onClick={triggerFile} className="px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ background: 'var(--teal)' }}>📂 Cargar archivo Excel</button>
      </div>
    )
  }
  ...
  const anteriorMap = useMemo(() => {
    ...
    const m = new Map<string, typeof state.anterior.products[0]>()
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...allRows].sort((a, b) => {
      ...
      if (['codigo','nombre','marca','ubicacion'].includes(sortCol as string)) {
        return dir * String((a.p as any)[sortCol] || '').localeCompare(String((b.p as any)[sortCol] || ''))
      }
      return dir * (((a.p as any)[sortCol] ?? 0) - ((b.p as any)[sortCol] ?? 0))
    })
  ...
  function thCls(col: SortCol) {
    return sortCol === col ? (sortDir === 'asc' ? 's-asc' : 's-desc') : ''
  }
  ...
  const ubicLabel = state.ubicaciones.length === 0 ? 'Todas' : state.ubicaciones.length === 1 ? shortUbic(state.ubicaciones[0]) : state.ubicaciones.length + ' ubicaciones'

  const costsUp = useMemo(() =>
    allRows.filter(r => r.costoAntIva != null && r.p.costoIva > r.costoAntIva)
  ...
  return (
    <div>
      ...
      <div className="rounded-2xl p-4 mb-4 flex items-center justify-between gap-4"
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-[0.68rem]" style={{ color: 'rgba(255,255,255,.68)' }}>Control y monitoreo de rentabilidad · Costos y precios con IVA (13%) · Solo productos con saldo &gt; 0</div>
          </div>
        </div>
      </div>

      {/* STATUS BAR */}
      <div className="flex items-center justify-between flex-wrap gap-2 px-3 py-2 rounded-[var(--r)] mb-4 text-[0.68rem]"
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          {state.anterior && <span><span className="inline-block w-1.5 h-1.5 rounded-full mr-1" style={{ background: 'var(--text3)' }} />Anterior: <b>{fmtDate(state.anterior.fecha)}</b></span>}
          <span>Ubicación: <b>{ubicLabel}</b></span>
          <span>Prods.: <b>{fmtN(prods.length)}</b></span>
        </div>
        <button onClick={appState.clearAll} className="text-[0.68rem] hover:text-red-500 transition-colors" style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
          🗑 Limpiar datos
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-2 mb-4">
        ...
        <KpiCard icon="📈" label="Margen prom."   value={fmtPct(kpis.margenProm)}   color={mgClass(kpis.margenProm) as any} />
      ...
      {/* SCATTER CHART */}
      <ScatterChart products={prods} criticos={kpis.criticos} medios={kpis.medios} sanos={kpis.sanos} />
      ...
      <div className="rounded-[var(--r)] p-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--sh-sm)', marginBottom: '0.65rem' }}>
        <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
          ...
          <div className="flex items-center gap-2 text-[0.69rem]" style={{ color: 'var(--text2)' }}>
            ...
            {hasFilters && (
              <button onClick={() => setFilters(EMPTY_FILTERS)} className="px-1.5 py-0.5 rounded text-[0.63rem] font-semibold border" style={{ background: 'var(--surface2)', borderColor: 'var(--border)' }}>
                ✕ Limpiar filtros
              </button>
            )}
            <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-7 gap-1.5 mb-3">
          {[
            { id: 'cod',  placeholder: 'Código…',      type: 'text' },
            { id: 'nom',  placeholder: 'Nombre…',      type: 'text' },
          ].map(f => (
            <input key={f.id} type="text" placeholder={f.placeholder}
              value={(filters as any)[f.id]}
              onChange={e => setFilters(prev => ({ ...prev, [f.id]: e.target.value }))}
              className="fi" style={{ padding: '0.33rem 0.55rem', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontSize: '0.69rem', width: '100%' }}
            />
          ))}
          <select value={filters.marca} onChange={e => setFilters(p => ({ ...p, marca: e.target.value }))}
            className="fi" style={{ padding: '0.33rem 0.55rem', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontSize: '0.69rem' }}>
            <option value="">Marca: Todas</option>
            {marcas.map(m => <option key={m} value={m}>{m}</option>)}
          </select>
          <select value={filters.ubic} onChange={e => setFilters(p => ({ ...p, ubic: e.target.value }))}
            style={{ padding: '0.33rem 0.55rem', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontSize: '0.69rem' }}>
            <option value="">Ubicación: Todas</option>
            {ubics.map(u => <option key={u} value={u} title={u}>{shortUbic(u)}</option>)}
          </select>
          <select value={filters.mg} onChange={e => setFilters(p => ({ ...p, mg: e.target.value }))}
            style={{ padding: '0.33rem 0.55rem', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontSize: '0.69rem' }}>
          ...
          <input type="number" step="0.1" placeholder="Mg. mín. %" value={filters.mgMin}
            onChange={e => setFilters(p => ({ ...p, mgMin: e.target.value }))}
            style={{ padding: '0.33rem 0.55rem', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontSize: '0.69rem', width: '100%' }} />
          <select value={filters.psug} onChange={e => setFilters(p => ({ ...p, psug: e.target.value }))}
            style={{ padding: '0.33rem 0.55rem', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontSize: '0.69rem' }}>
        ...
        <div style={{ maxHeight: 480, overflow: 'auto', borderRadius: 8, border: '1px solid var(--border)' }}>
          <table>
            <thead>
              <tr>
                {[
                  { col: 'codigo',      label: 'Código',          cls: '' },
                  { col: 'nombre',      label: 'Nombre',          cls: '' },
                  { col: 'marca',       label: 'Marca',           cls: '' },
                  { col: 'ubicacion',   label: 'Ubicación',       cls: '' },
                  { col: 'existencia',  label: 'Existencia',      cls: 'tr' },
                  { col: 'costoIva',    label: 'Costo c/IVA',     cls: 'tr' },
                  { col: 'precioSug',   label: '💡 P. Sugerido', cls: 'tr' },
                  { col: 'precioIva',   label: 'Precio c/IVA',    cls: 'tr' },
                  { col: 'margen',      label: 'Margen %',        cls: 'tr' },
                  { col: 'utilidadUnit','label': 'Utilidad Unit.', cls: 'tr' },
                  { col: 'utilidadTotal','label':'Utilidad Total', cls: 'tr' },
                ].map(h => (
                  <th key={h.col} onClick={() => sort(h.col as SortCol)} className={`${h.cls} ${thCls(h.col as SortCol)}`}>
            ...
            <tbody>
              {filtRows.slice(0, 600).map(({ p, costoAntIva }) => {
                const ps   = precioSugerido(p.costoIva)
                const diff = p.precioIva - ps
      ...
      {anteriorMap && anteriorMap.size > 0 && (
        <div className="rounded-[var(--r)] p-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--sh-sm)' }}>
          ...
          {costsUp.length === 0
            ? <div className="text-[0.76rem] text-center py-4" style={{ color: 'var(--text2)' }}>Ningún costo aumentó respecto al archivo anterior.</div>
            : (
              <div style={{ maxHeight: 280, overflow: 'auto', borderRadius: 8, border: '1px solid var(--border)' }}>
                <table>
                  <thead><tr>
                    ...
                    <th className="tr">Saldo</th><th className="tr">Costo ant. c/IVA</th>
                    <th className="tr">Costo act. c/IVA</th><th className="tr">Variación %</th>
                  </tr></thead>
                  <tbody>
                    {costsUp.slice(0, 80).map(({ p, costoAntIva, variacion }) => (
                      <tr key={p.codigo}>
                        <td style={{ fontFamily: 'monospace', fontSize: '0.64rem' }}>{p.codigo}</td>
